// src/modules/rbac/profile/repositories/profile.repository.ts
import { AppDataSource } from "../../../../init";
import { IProfileRepository, IProfile } from "../interfaces/profile.interface";
...
export class ProfileRepository implements IProfileRepository {
    ...
    public async getByEmpId(empId: string): Promise<UserModel | null> {
        try {
            const userRecord = await this.userModel.findOne({ where: { empId } });
            return userRecord || null;
        } catch (error) {
            console.error("Error in getByEmpId:", error);
            throw new Error("Error retrieving user by empId");
        }
    }
    ...
    //     try {
    //         const providerRepo = queryRunner ? queryRunner.manager.getRepository(ProviderModel) : this.providerModel;
    //         const newProvider = providerRepo.create(providerData as any);
    ...
    public async getAllUsersWithRolesPaginated(
        ...
        try {
            ...
            // empId filter (if provided)
            if (empId) {
                whereClauses.push(`u."empId" = $${params.length + 1}`);
                params.push(empId);
            }
            ...
            const countQuery = `
               ...
               FROM rms_tech_solutions_db.public.users u
               ${whereSQL}
            `;
    ...
    public async update(userId: string, data: IProfile): Promise<any> {
        try {
            const query = `
